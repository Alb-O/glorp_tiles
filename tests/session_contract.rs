...
use libtiler::{
    Axis, BalancedPreset, Direction, LeafMeta, NavError, OpError, PresetKind, ResizeStrategy,
    Session, Slot, SolverPolicy, TallPreset, ValidationError, WeightPair,
};
...
fn split_ids_postorder_returns_descendants_before_ancestors() {
    ...
    session
        .set_selection(a)
        .expect("select left leaf for nested split");
    let _ = session
...
fn apply_preset_noops_do_not_bump_revision() {
    ...
    leaf_selected
        .validate()
        .expect("leaf no-op should stay valid");
    ...
    matching
        .validate()
        .expect("matching no-op should stay valid");
}
...
fn apply_preset_preserves_focus_and_retargets_selection_to_rebuilt_subtree() {
    ...
    session
        .set_focus_leaf(preserved_leaf)
        .expect("focus preserved leaf");
    let revision = session.revision();
...
fn apply_preset_nested_selection_retargets_rebuilt_subtree_and_root_id_stays() {
    ...
    let nested = session
        ...
        .expect("nested split should exist");
    session
        .set_selection(nested)
        .expect("select nested subtree for preset");
    session
        .set_focus_leaf(preserved_leaf)
        .expect("focus preserved leaf");
    let revision = session.revision();
    ...
    let rebuilt = session
        .selection()
        .expect("selection should retarget to rebuilt");
    assert_eq!(session.focus(), Some(preserved_leaf));
    assert_eq!(session.selection(), Some(rebuilt));
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(root),
        "root id stays the same"
    );
    assert_eq!(session.revision(), revision + 1);
    session
        .validate()
        .expect("nested rewrite should stay valid");
}
...
fn move_selection_as_sibling_of_preserves_focus_and_selected_subtree() {
    ...
    session
        ...
        .expect("select nested subtree to move");
    session
        .set_focus_leaf(preserved_leaf)
        .expect("focus preserved leaf");
    let revision = session.revision();
